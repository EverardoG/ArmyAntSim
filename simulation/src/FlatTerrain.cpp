...
FlatTerrain::FlatTerrain(b2World* world, config::sWindow windowParam, config::sTerrain terrainParam, double bodyLength)
...
void FlatTerrain::createBody(b2World* world){
...
//	// std::cout << "FlatTerrain::createBody() " << m_posGoal.x << std::endl;
//
...
void FlatTerrain::drawBody(sf::RenderWindow& window){
...
void FlatTerrain::drawBody(sf::RenderTexture& texture){
	// std::cout << "for (Polygon polygon : allPolygons) {" << std::endl;
...
e_terrain_type FlatTerrain::getType(){
	return ISLAND;
}
